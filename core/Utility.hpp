...
struct Grid {
  Grid(const Pos& argSize, int argHSize, int argVSize)
      : size(argSize), hsize(argHSize), vsize(argVSize), cell(size.x / hsize, size.y / vsize) {
    hspace = 0;
    vspace = 0;
  }
...
struct Color {
  ...
  Color(float argR, float argG, float argB, float argA = 1.0) : r(argR), g(argG), b(argB), a(argA) {
  }
