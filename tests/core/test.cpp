...
int GliderAppMain(int argc, char* argv[]) {
  ...
  try {
    ...
    Text::Ref t = new Text(engine.getDefaultFont(), str);
    Font::Ref glow = manager.getFont(engine.getDefaultFont()->getFileName().c_str(), engine.getDefaultFont()->getSize(),
                                     engine.getDefaultFont()->getFlags() | ffGlow);
    Text::Ref t2 = new Text(glow, str);
    sc.addObject(t2.get());
    sc.addObject(t.get());
    ...
    Text* rus = new Text(meh.fnt1.get(),
                         "BLAH! это тест по русски\nsdf ^rkasnd^< kfajsnd ^bfkasjnd^< ^gfkas^< dnfkasdnf kasdnkf "
                         "^359WEIRD^< aksdnjf kasjdn fkasj ndfkasjn dfkasndfka",
                         false, 200, true);
    rus->setPosition(Pos(50.0f, 100.0f));
    glider::Rectangle* rr2 = new glider::Rectangle(
        Rect(Pos(50.0f, 100.0f), Pos((float)rus->getWidth(), (float)rus->getHeight())), Color(0.2f, 0.2f, 0.2f));
    sc.addObject(rr2);
    sc.addObject(rus);
