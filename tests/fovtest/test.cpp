...
public:
  ...
  void onMouseEvent(MouseEvent& argEvent) {
    ...
    if (argEvent.eventType == metButtonPress && argEvent.eventButton == 4) {
      if (viewAngle < AngleRange::SCALED_PI - AngleRange::SCALED_PI / 18)
        viewAngle += AngleRange::SCALED_PI / 90;
      updateDr = true;
    }
    if (argEvent.eventType == metButtonPress && argEvent.eventButton == 5) {
      if (viewAngle > AngleRange::SCALED_PI / 18)
        viewAngle -= AngleRange::SCALED_PI / 90;
      updateDr = true;
    }
  ...
  void onKeyboardEvent(KeyboardEvent& argEvent) {
    ...
    if (argEvent.eventType == ketPress) {
      auto ox = px;
      auto oy = py;
      if (argEvent.keySym == keyboard::GK_UP || argEvent.keySym == keyboard::GK_W)
        py--;
      if (argEvent.keySym == keyboard::GK_DOWN || argEvent.keySym == keyboard::GK_S)
        py++;
      if (argEvent.keySym == keyboard::GK_LEFT || argEvent.keySym == keyboard::GK_A)
        px--;
      if (argEvent.keySym == keyboard::GK_RIGHT || argEvent.keySym == keyboard::GK_D)
        px++;
      if (argEvent.keySym == keyboard::GK_KP_PLUS) {
        sightRadius++;
        ox = -1;
      }
      if (argEvent.keySym == keyboard::GK_KP_MINUS && sightRadius > 2) {
        sightRadius--;
        ox = -1;
      }
...
int GliderAppMain(int argc, char* argv[]) {
  ...
  Scene sc;
  Font::Ref fnt1 = manager.getFont("FSEX300.ttf", 16);
