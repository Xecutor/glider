...
void EditInput::onFocusGain() {
  LOGDEBUG(log, "gain:name=%{} focus=%{}, ani.active=%{}", name, focused ? "true" : "false",
           curBlinkAni.active ? "true" : "false");
...
void EditInput::onFocusLost() {
  ...
  LOGDEBUG(log, "lost:name=%{} focus=%{}, ani.active=%{}", name, focused ? "true" : "false",
           curBlinkAni.active ? "true" : "false");
}
...
void EditInput::deleteSymbol(int dir) {
  if (curPos + dir >= 0 && curPos + dir < text.getTextLength()) {
    ...
    if (eiCb[eiOnModify])
      eiCb[eiOnModify](UIEvent(this, eiOnModify));
  }
}
...
void EditInput::setCurPos(size_t argCurPos, bool extendSelection) {
  ...
  if (extendSelection && oldCurPos != curPos) {
    ...
    } else if (oldCurPos == selStart) {
      selStart = curPos;
    } else if (oldCurPos == selEnd) {
      selEnd = curPos;
    }
    if (selEnd < selStart) {
