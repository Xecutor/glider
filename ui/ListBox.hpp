...
enum ListBoxEventType { lbetSelectionChanged, lbetCount };

class ListBox : public UIContainer {
public:
  using Ref = ReferenceTmpl<ListBox>;
  ListBox();
  void draw();
...
protected:
  UIObjectsList::iterator topItem = objLst.end();
  UIObjectsList::iterator selItem = objLst.end();
