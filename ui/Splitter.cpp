...
Splitter::Splitter(SplitterOrientation argOrientation, int argPosition) : orientation(argOrientation), position(0) {
...
void Splitter::setFirst(UIObject::Ref obj) {
  if (first.get()) {
...
void Splitter::setSecond(UIObject::Ref obj) {
  if (second.get()) {
...
void Splitter::updateSecondPosAndSize() {
  ...
  LOGDEBUG("ui.splitter", "second(%{}):%{} pos=%{} size=%{}", getName(), second->getName(), second->getPos(),
           second->getSize());
}
