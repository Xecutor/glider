...
void UIContainer::addObject(UIObject::Ref obj) {
...
void UIContainer::removeObject(UIObject::Ref obj) {
  if (obj->isFocused()) {
    obj->removeFocus();
  ...
  idMap.erase(it);
  if (!mouseEnterStack.empty() && mouseEnterStack.back() == obj) {
    mouseEnterStack.pop_back();
  }
}

UIObject::Ref UIContainer::findByName(const std::string& argName) {
  ...
  auto uic = it->second.as<UIContainer>();
  return uic->findByName(argName.substr(dotPos + 1));
}

void UIContainer::moveObjectToFront(UIObject::Ref obj) {
  IdMap::iterator it = idMap.find(obj->getUid());
  if (it == idMap.end()) {
    //!!TODO!! throw exception?
    return;
